...
Human (Slack / WebEx / WebChat) --> NetClaw (CCIE Agent on OpenClaw)
                                ...
                                |     MCP: Infrahub        --> Schema-driven SoT, GraphQL, branch-isolated writes (10 tools)
...
│       ├── infrahub-sot/               # Infrahub schema-driven SoT, GraphQL, branch-isolated writes
...
11. **Installs Infrahub MCP** — `pip3 install infrahub-mcp` (falls back to `git clone` + `uv sync`) for OpsMill Infrahub schema-driven source of truth (10 tools: nodes, search, GraphQL, and branch-isolated writes via Proposed Changes). Requires Python 3.13+ and an Infrahub instance with API token. Launched at runtime via `uvx infrahub-mcp`; also available on PyPI and as a Docker image (`registry.opsmill.io/opsmill/infrahub-mcp`).
